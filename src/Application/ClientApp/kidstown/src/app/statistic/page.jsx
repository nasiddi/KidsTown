...
import { fetchLocationGroups, fetchLocations, post } from '../helpers/BackendClient'
...
function Statistic() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const selectedLocations = getSelectedOptionsFromStorage('statisticLocations', [])
      setState({
        ...state,
  ...
  useEffect(() => {
    async function fetchData(locations) {
      ...
      return await post(
        ...
        locations.flatMap((l) => l.options.filter((o) => o.isSelected).map((o) => o.value)),
      )
        .then((r) => r.json())
        .then((j) => {
          return j
  ...
  function onLocationGroupSelect(event) {
    const locations = state.allLocations.find((l) => l.groupId === parseInt(event.target.id, 10))
  ...
  function onLocationGroupDeselect(event) {
    ...
    const selectedLocations = state.selectedLocations.filter((l) => l.groupId !== parseInt(id, 10))
  ...
  const noMoreDataAvailable = !state.attendees.some((a) => a.date.startsWith(state.furthestYear))
