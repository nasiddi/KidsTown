...
namespace KidsTown.BackgroundTasks.PlanningCenter
{
    public class UpdateService : IUpdateService
    {
        ...
        private async Task UpdateParents(IImmutableList<Family> families)
        {
        ...
        private async Task UpdateLocations(IImmutableList<CheckIns> checkIns)
        {
            ...
            await _updateRepository.UpdateLocations(locationUpdates: newLocations.Select(selector: l
                    => MapLocationUpdate(
                        location: l,
        ...
        private async Task<IImmutableList<Family>> UpdatePeople()
        {
            var peopleIds = await _updateRepository.GetCurrentPeopleIds(daysLookBack: DaysLookBack)
                .ConfigureAwait(continueOnCapturedContext: false);
            ...
            var newFamilies =
                await _updateRepository.InsertFamilies(newHouseholdIds: newHouseholdIds, peoples: peopleUpdates);

            var families = existingFamilies.Union(second: newFamilies).ToImmutableList();

            await _updateRepository.UpdateKids(kids: peopleUpdates, families: families)
                .ConfigureAwait(continueOnCapturedContext: false);
        ...
        private async Task InsertNewPreCheckIns(IImmutableList<CheckInsUpdate> preCheckIns)
        {
            ...
            await _updateRepository.InsertPreCheckIns(preCheckIns: newCheckins)
                .ConfigureAwait(continueOnCapturedContext: false);
        }

        private static IImmutableList<PeopleUpdate> MapPeopleUpdates(IImmutableList<Peoples> peopleUpdates)
        {
        ...
        private static LocationUpdate MapLocationUpdate(
            PlanningCenterApiClient.Models.CheckInsResult.Included location,
